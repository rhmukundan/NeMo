...
def mcore_model_customize(cfg, model):
    if cfg.get("apply_embedding_scaling", False) and parallel_state.is_pipeline_first_stage(
        ignore_virtual=not model.enable_virtual_pipeline_model_parallel,
...
class MegatronGPTModel(MegatronBaseModel, TextGeneration):
    ...
    def allreduce_first_last_embeddings(self):
        ...
        if parallel_state.get_pipeline_model_parallel_world_size() > 1 and (
            ...
            module_list = self.get_model_module_list()
            if parallel_state.is_pipeline_first_stage():
                module = module_list[0]  # only the first pipline rank has the embeddings
            elif parallel_state.is_pipeline_last_stage():
                module = module_list[-1]  # only the last pipline rank has the embeddings
            share_embeddings = (
