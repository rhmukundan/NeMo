...
class MegatronVitClassificationModel(MegatronBaseModel):
    ...
    def training_step(self, dataloader_iter):
        ...
        if self.with_distributed_adam:
            ...
            # reduced
            if not parallel_state.is_pipeline_first_stage(ignore_virtual=False, vp_stage=getattr(self.model, 'vp_stage', None)):
    ...
    def get_forward_output_and_loss_func(self, validation_step=False):
        ...
        def fwd_output_and_loss_func(dataloader_iter, model):
            ...
            if parallel_state.get_pipeline_model_parallel_world_size() == 1:
                ...
                if parallel_state.is_pipeline_first_stage(ignore_virtual=False, vp_stage=getattr(self.model, 'vp_stage', None)):
                    # Fist pipeline stage needs only the tokens and position_ids
                    tokens = batch[0].cuda(non_blocking=True)
                    labels = None
                elif parallel_state.is_pipeline_last_stage(ignore_virtual=False, vp_stage=getattr(self.model, 'vp_stage', None)):
    ...
    def on_validation_epoch_end(self):
        ...
        if parallel_state.is_pipeline_last_stage(ignore_virtual=False, vp_stage=getattr(self.model, 'vp_stage', None)):
